...
def enhancedFeatureExtractorDigit(datum):
    ...
    #  lap qua tat ca cac pixel
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            # Neu khong thay duoc pixel , xu li no!
            if (x, y) not in visited_pixels and features[(x, y)] == 0:
                # Danh dau pixel
                visited_pixels.add((x, y))

                # tang so luong cac vung trang
                connected_regions += 1
...
def enhancedPacmanFeatures(state, action):
    ...
    features['towardsGhost'] = newDisToNearestGhost < util.manhattanDistance(
        pos, nearestGPos)
